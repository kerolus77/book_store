...
class HomeRepoImpl implements HomeRepo {
 ...
 final CartRepoImpl cartRepoImpl;
  ...
  Future<Either<Failure, List<BookModel>>> fetchForYouBooks() async {
    try {
      ...
      for (var item in data['items']) {
        ...
        favoriteRepoImpl.favoriteId.contains(books[books.length-1].id)?books[books.length-1].isFavorite=1:null;
        cartRepoImpl.cartId.contains(books[books.length-1].id)?books[books.length-1].isInCart=1:null;
      }
      return right(books);
    } catch (e) {
  ...
  try {
    ...
    print(favoriteRepoImpl.favoriteId.length);
     for (var item in data['items']) {
        freeBooks.add(BookModel.fromJson(item));
      favoriteRepoImpl.favoriteId.contains(freeBooks[freeBooks.length-1].id)?freeBooks[freeBooks.length-1].isFavorite=1:null;
      cartRepoImpl.cartId.contains(freeBooks[freeBooks.length-1].id)?freeBooks[freeBooks.length-1].isInCart=1:null;
      }
  ...
  Future<Either<Failure, List<BookModel>>> fetchCategoryBooks(String category)async {
    try {
      ...
      favoriteRepoImpl.favoriteId.contains(categoryBooks[categoryBooks.length-1].id)?categoryBooks[categoryBooks.length-1].isFavorite=1:null;
      cartRepoImpl.cartId.contains(categoryBooks[categoryBooks.length-1].id)?categoryBooks[categoryBooks.length-1].isInCart=1:null;
      }
