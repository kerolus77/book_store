 ...
 void initState() {
           ...
           builder: (context) {
             return Stack(
              alignment: Alignment.bottomRight,
               children: [
                 pageView(),
                BottomContainer(currentIndex: _currentIndex, pages: pages, context: context),
                skipButton(),
               ],
             )
             ;
  ...
  TextButton skipButton() {
    return TextButton(onPressed: (){
      Get.to(const Home(),transition: Transition.fadeIn,duration:  transitionDuration);
    }, child:Text('Skip',style: Themes().subheadingStyle.copyWith(color:Colors.black),));
  }
          ...
          builder: (context) {
            return PageView(
                ...
                children: pages.map((page) =>  Stack(
                  children: [
                      Positioned.fill(
                      bottom: SizeConfig.orientation==Orientation.landscape?0:MediaQuery.of(context).size.height/3.5,
                      right:SizeConfig.orientation==Orientation.landscape?MediaQuery.of(context).size.width/2:0 ,
                    child: Image(
